var ngRoot=angular.module('starter', ['ionic'])
...
ngRoot.controller("ClickToEditCtrl", ['$scope',function($scope) {    
	 	var getRandomInt=function(min, max){
		    return Math.floor(Math.random() * (max - min + 1)) + min;
		};

		var chooseDictionary=function (){
		    var randomIndex=getRandomInt(1,150);
		    while($scope.chooseIndex.indexOf(randomIndex)!=-1)
		    {
		    	  randomIndex=getRandomInt(1,150);
		    }
		    $scope.chooseIndex.push(randomIndex);
		    
		    var randomTitle =  $scope.title.filter(function(item) {
		    	return item.number == randomIndex
				});
		    randomTitle[0]["randomFlag"]=true;
		    
		    var currentTitle =  $scope.title.filter(function(item) {
		    	return item.number == $scope.title2[0]["number"];
				});
		...
		$scope.nextDictionary = function(){
		    ...
		    if(randomDictionary!=undefined){
		    	...
		    	$scope.title2=[];
		    	$scope.title2.push(randomDictionary);
		    	$scope.showEnglish=false; 
			    var exist =  $scope.title.filter(function(item) {
		...
		$scope.finishFlag=false;
	  	$scope.title2=[{"number":1,"eg":"are","ch":"be動詞","txtEnglish":"","txtChinese":"","randomFlag":false}];
	  	$scope.title = [{"number":1,"eg":"are","ch":"be動詞","txtEnglish":"","txtChinese":"","randomFlag":false}
